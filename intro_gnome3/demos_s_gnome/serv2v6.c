 ...
 * serv2v6.c
 *
 * Command line server for UDP IPv6 with reading timeout
 *
...
int main (int argc, char *argv[])
{
  ...
  name.sin6_addr = in6addr_any;	/* Default local IPv6 - all interfaces */
  ...
  if (recvfrom (sock, buf, 1024, 0 /* By default it waits for data */  ,
    ...
    if (errno == EWOULDBLOCK) {
    	printf("Timeout while waiting for datagram packet\n");
    } else {
